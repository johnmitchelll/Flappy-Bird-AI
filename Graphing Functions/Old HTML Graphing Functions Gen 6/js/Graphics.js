...
function drawGraph(){   
    ...
    for (var i = 0; i < graphs.length; i++) {
        ...
        drawDomainOutput(graphs[i].domain, graphs[i].color);
    ...
    if(prevOrientation != getDeviceOrientation()){
...
function drawNumberMarks(xPos, yPos){
        for (var i = 0; i < yPos.length; i++) {
            let height = 6;
            let width = canvasContext.measureText(-yPos[i]).width;
            drawText("white", -yPos[i], -canvas.width/2+width/2, 
                     yPos[i]/zoom * canvas.height+offY * canvas.height/zoom+height/2)
            drawText("white", -yPos[i], canvas.width/2-width*1.5, 
                     yPos[i]/zoom * canvas.height+offY * canvas.height/zoom+height/2)
        }
        ...
        for (var i = 0; i < xPos.length; i++) {
            // draw numbers 
            let width = canvasContext.measureText(xPos[i]).width;
            drawText("white", xPos[i],xPos[i]/zoom * canvas.width + offX * canvas.width/zoom - width/2,
                     canvas.height/2-3)
            drawText("white", xPos[i],xPos[i]/zoom * canvas.width + offX * canvas.width/zoom - width/2,
                     -canvas.height/2+9)
        }
...
function drawTangentLine(graph){
    ...
    console.log(mouseXAfterZoom)
    let outPutAtMouseX = graph.func(mouseXAfterZoom);
    let dAtMouseX = FuncDerivitive(mouseXAfterOffset, graph.domain);
    ...
    for (var i = 0; i < linearDomain.length; i++) {
        ...
        drawLine(temp - linearDomain.length/2+mouseX, yOfTempInRealSpace, 
                i - linearDomain.length/2+mouseX, yOfIInRealSpace, 4,'white');

        if(i > 0){temp++;}
    }

    slope = dAtMouseX.d / dAtMouseX.dx;
    colorCircle(mouseX-linearDomain.length/2, (linearDomain[0] / zoom) * canvas.height + offY * canvas.height/zoom, 7, graph.color);
    colorCircle(mouseX+linearDomain.length/2, (linearDomain[linearDomain.length-1] / zoom) * canvas.height + offY * canvas.height/zoom, 7, graph.color);
    colorCircle(mouseX, (linearDomain[Math.floor(linearDomain.length/2)] / zoom) * canvas.height + offY * canvas.height/zoom, 7, "white");

    graph.tanEquals.innerHTML = "= " + (-slope).toFixed(2);
}
...
function drawAreaUnderCurve(graph){
    ...
    for (var i = 0; i < c.length; i++) {
        ...
        outPutsAtC.push(graph.domain[indexAtC + Math.floor(canvas.width/2)]);
    }
    ...
    graph.defIntegral.innerHTML = "= " + (calculateAreaUnderCurve(c, deltaX, graph)).toFixed(2);
