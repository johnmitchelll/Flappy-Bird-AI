function handleTurn(){
    if(currentTurn == "Computer" && cpuWordle.movesLeft > 0){
        computerMakeBestMove();          
    }
}

function addWordToRow(word, wordle){
    wordle.wordsUsed.push(word)

    for (var i = 0; i < word.length; i++) {
        wordle.grid[wordle.currentIndex.x][wordle.currentIndex.y].letter.letter = word[i];

        if(wordle.currentIndex.x == wordle.cols-1){
            wordle.checkRow();
            wordle.checkWinner();
            wordle.nextRow();
            currentTurn = "Player";
        }else{
            wordle.currentIndex.x++;
        }
    }
}
