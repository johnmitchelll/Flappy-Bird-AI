        ...
        for (var col = 0; col < CELL_COLS; col++) {
         for (var row = 0; row < CELL_ROWS; row++) {
            ...
            if(letters[j].g[col + CELL_COLS * row] == 1){
                  for (var scalecol = 0; scalecol < scale; scalecol++) {
                    for (var scalerow = 0; scalerow < scale; scalerow++) {

                      brickGrid[(i+2+i+i+(i * CELL_COLS * scale)+(col*scale))+scalecol]
                                [((BUFFER_ROWS/scale)+(row*scale))+scalerow].target = true;

                      targetList.push(brickGrid[(i+2+i+i+(i * CELL_COLS * scale)+(col*scale))+scalecol]
                                [((BUFFER_ROWS/scale)+(row*scale))+scalerow]);
